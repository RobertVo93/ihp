...
const ClinicOverviewVisitsTable = (props) => {
	...
	useEffect(() => {
		let newTableData = [];
		if (props.procedures == null || props.procedures.length <= 0) return;
		for (var i = 0; i < props.procedures.length; i++) {
			newTableData.push({
				no: i + 1,
				IOTDesc: props.procedures[i].IOTDesc,
				Price:
					"$" +
					(props.procedures[i].Price == null
						? 0
						: moneyFormat(props.procedures[i].Price)),
				IOTType: props.procedures[i].IOTType,
				Qty: props.procedures[i].Qty,
			});
		}
