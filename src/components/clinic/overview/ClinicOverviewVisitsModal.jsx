...
import ClinicOverviewDrugsTable from "./ClinicOverviewDrugsTable";
import ClinicOverviewProceduresTable from "./ClinicOverviewProceduresTable";
const { getClaimDetailsAndHistory } = CMActions;
const { getDrugsAndProcedures } = ClinicActions;

const ClinicOverviewVisitsModal = props => {
	...
	const currentVisit = props.clinicData.visits.find(
					...
					<Card>
						<CardBody>
							...
							<ClinicOverviewProceduresTable
								procedures={props.clinicData.visits.filter(
									val =>
										val.ROCNo === props.visitId &&
										val.IOTType === "O" &&
										val.VType === currentVisit.VType
