...
import CompanyProductivityTopCHTable from "./CompanyProductivityTopClinicHoppersTable";
import { useHistory } from "react-router-dom";
import ChartLoading from "views/ChartLoading";
import customTooltip from "components/common/chartTooltip";

const { getCompanyProductivityTopClinicHoppers } = CompActions;

const CompanyProductivityTopCH = props => {
  //call apis
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      await props.getCompanyProductivityTopClinicHoppers(
        props.authData,
  ...
  useEffect(() => {
    setIncData(props.companyData.topClinicHoppers);
  }, [props.companyData.topClinicHoppers]);
  ...
  useEffect(() => {
    ...
    for (var i = 0; i < incomingData.length; i++) {
      tData.push({
        ...
        ClinicCount: incomingData[i].ClinicCount,
      ...
      if (i <= 5) {
        ...
        data.push(incomingData[i].ClinicCount);
          ...
          <div className="roe-card-header">
            <div className="row header-container">
                <div className="float-left">
                    <p className="card-main-header">Top Clinic Hoppers</p>
      ...
      <div className="col-lg-6">
        <div className="roe-card-style mt-15 mb-30">
          <div className="roe-card-header">
            <div className="row header-container">
                <div className="float-left">
                    <p className="card-main-header">Top Clinic Hoppers (Table)</p>
                    <p className="card-sub-header">Overview of all time</p>
                </div>
            </div>
          </div>
          {(incomingData != null && incomingData.length) > 0 && (
            <CompanyProductivityTopCHTable tData={tableData} />
          )}
          {(incomingData == null || incomingData.length == 0) && (
            <div style={{ marginLeft: "23px", marginBottom: "15px" }}>
              <p>This chart contains no data</p>
...
const mapStateToProps = state => {
  return {
    ...
    companyData: {
      ...
      topClinicHoppers: state.company.companyProductivityTopClinicHoppers
    }
  };
};

export default connect(mapStateToProps, {
  getCompanyProductivityTopClinicHoppers
})(CompanyProductivityTopCH);
