...
const AMTopBrokers = props => {
	const [tableData, setTableData] = useState([]);

	useEffect(() => {
		//update the data
		props.getTopBrokers(props.authData);
	}, []);

	useEffect(() => {
		//update the data
		if (props.amData.topBrokers == null || props.amData.topBrokers.length == 0)
			return;
		let newTableData = [];
		for (var i = 0; i < props.amData.topBrokers.length; i++) {
			newTableData.push({
				no: i + 1,
				CompanyName: props.amData.topBrokers[i].CompanyName,
				Lives: numberFormat(props.amData.topBrokers[i].ECount)
			});
		}
		setTableData(newTableData);
	}, [props.amData.topBrokers]);
		...
		<ReactTableWrapper {...props}>
			<div className="roe-card-style mt-15 mb-30">
				<div className="roe-card-header">
					<span className="hash"> </span> Top Brokers
        		</div>
...
const mapStateToProps = state => {
	return {
		...
		amData: {
			topBrokers: state.accountmanagers.topBrokers
		}
	};
};

export default connect(mapStateToProps, { getTopBrokers })(AMTopBrokers);
